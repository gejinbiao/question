...
# 二叉查找树
首先，让我们先看一张图：
![NfVuzd.png](https://s1.ax1x.com/2020/06/29/NfVuzd.png)
...
![NfVwyn.png](https://s1.ax1x.com/2020/06/29/NfVwyn.png)
...
下面是平衡二叉树和非平衡二叉树的对比：
![NfVoTK.png](https://s1.ax1x.com/2020/06/29/NfVoTK.png)
...
我们都知道平衡二叉树可是每个节点只存储一个键值和数据的。那说明什么？说明每个磁盘块仅仅存储一个键值和数据！那如果我们要存储海量的数据呢？
  ...
  可以想象到二叉树的节点将会非常多，高度也会极其高，我们查找数据时也会	进行很多次磁盘 IO，我们查找数据的效率将会极低！

![NfZSTf.png](https://s1.ax1x.com/2020/06/29/NfZSTf.png)

为了解决平衡二叉树的这个弊端，我们应该寻找一种单个节点可以存储多个键值和数据的平衡树。也就是我们接下来要说的 B 树。

B 树（Balance Tree）即为平衡树的意思，下图即是一棵 B 树：
![NfZPfg.png](https://s1.ax1x.com/2020/06/29/NfZPfg.png)
...
![NfZMhF.png](https://s1.ax1x.com/2020/06/29/NfZMhF.png)
...
## **利用聚集索引查找数据**
![NfewV0.png](https://s1.ax1x.com/2020/06/29/NfewV0.png)
...
![Nfe2rR.png](https://s1.ax1x.com/2020/06/29/Nfe2rR.png)

## 利用非聚集索引查找数据
![NfmAZq.png](https://s1.ax1x.com/2020/06/29/NfmAZq.png)

读者看到这张图的时候可能会蒙，这是啥东西啊？怎么都是数字。如果有这种感觉，请仔细看下图中红字的解释。
什么？还看不懂？那我再来解释下吧。首先，这个非聚集索引表示的是用户幸运数字的索引（为什么是幸运数字？一时兴起想起来的:-)），此时表结构是这样的。

![NfmnWF.png](https://s1.ax1x.com/2020/06/29/NfmnWF.png)
...
下面看下具体的查找流程图：
![NfmsTP.png](https://s1.ax1x.com/2020/06/29/NfmsTP.png)
